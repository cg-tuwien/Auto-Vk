	...
	public:
		...
		struct presets
		{
			...
			struct image_copy
			{
				...
				static ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<read_memory_access>)> wait_for_previous_operations(ak::image_t& aSourceImage, ak::image_t& aDestinationImage);
				...
				static ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<write_memory_access>)> let_subsequent_operations_wait(ak::image_t& aSourceImage, ak::image_t& aDestinationImage);
				...
				static ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<write_memory_access>)> directly_into_present(ak::image_t& aSourceImage, ak::image_t& aDestinationImage);
			};
		};
		...
		static bool is_about_to_steal_before_handler_on_demand(const ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<read_memory_access>)>& aToTest) {
		...
		static bool is_about_to_steal_after_handler_on_demand(const ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<write_memory_access>)>& aToTest) {
		...
		static bool is_about_to_steal_before_handler_immediately(const ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<read_memory_access>)>& aToTest) {
			const auto trgPtr = aToTest.target<steal_before_handler_t>();
			return nullptr == trgPtr ? false : *trgPtr == steal_before_handler_immediately ? true : false;
		}
		static bool is_about_to_steal_after_handler_immediately(const ak::unique_function<void(command_buffer_t&, pipeline_stage, std::optional<write_memory_access>)>& aToTest) {
			const auto trgPtr = aToTest.target<steal_after_handler_t>();
...
#pragma region static creation functions
		 ...
		 *
		 *	Example usage:
		 *	ak::sync::with_semaphore([](ak::semaphore s) { ak::context().main_window()->set_extra_semaphore_dependency(std::move(s)); }
		 */
		template <typename F>
		 ...
		 *	
		 *	@param	aEstablishBarrierBeforeOperation	Function signature: void(ak::command_buffer_t&, ak::pipeline_stage, std::optional<ak::read_memory_access>)
		 ...
		 *												
		 *	@param	aEstablishBarrierAfterOperation		Function signature: void(ak::command_buffer_t&, ak::pipeline_stage, std::optional<ak::write_memory_access>)
		...
		static sync with_barriers_by_return(
			ak::unique_function<void(command_buffer_t&, pipeline_stage /* destination stage */, std::optional<read_memory_access> /* destination access */)> aEstablishBarrierBeforeOperation = {},
			ak::unique_function<void(command_buffer_t&, pipeline_stage /* source stage */,	  std::optional<write_memory_access> /* source access */)> aEstablishBarrierAfterOperation = presets::default_handler_after_operation)
		{
			sync result;
		...
		static sync auxiliary_with_barriers(
			sync& aMasterSync,
			ak::unique_function<void(command_buffer_t&, pipeline_stage /* destination stage */, std::optional<read_memory_access> /* destination access */)> aEstablishBarrierBeforeOperation,
			ak::unique_function<void(command_buffer_t&, pipeline_stage /* source stage */, std::optional<write_memory_access> /* source access */)> aEstablishBarrierAfterOperation
		);
#pragma endregion
...
#pragma endregion
	...
	private:
		...
		ak::unique_function<void(semaphore)> mSemaphoreLifetimeHandler;
		std::vector<semaphore> mWaitBeforeSemaphores;
		std::variant<std::monostate, ak::unique_function<void(command_buffer)>, std::reference_wrapper<command_buffer_t>> mCommandBufferRefOrLifetimeHandler;
