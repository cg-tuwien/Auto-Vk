...
namespace ak
{
	...
	static std::vector<char> load_binary_file(std::string path)
	{
		...
		if (!is) {
			throw ak::runtime_error("Couldn't load file '" + path + "'");
		}
	...
	public:
		...
		owning_resource(const owning_resource<T>& other)
		{
			if (other.has_value()) {
				if (!other.is_shared_ownership_enabled()) {
					...
					//
					throw ak::logic_error(fmt::format("You are trying to copy-construct a resource of type {} which does not have shared ownership enabled. This call will fail now. You can try to use owning_resource::enable_shared_ownership().", typeid(T).name()));
				}
		...
		owning_resource<T>& operator=(const owning_resource<T>& other)
		{
			if (other.has_value()) {
				if (!other.is_shared_ownership_enabled()) {
					...
					//	An alternative to initializer lists would be to use `ak::make_vector`.
					//
					throw ak::logic_error("Can only copy assign owning_resources which have shared ownership enabled.");
				}
				*this_as_variant() = std::get<std::shared_ptr<T>>(other);
			}
			else {
		...
		void enable_shared_ownership()
		{
			...
			if (std::holds_alternative<std::monostate>(*this)) {
				throw ak::logic_error("This owning_resource is uninitialized, i.e. std::monostate.");
			}
	...
	class unique_function : protected std::function<T>
	{
											 ...
											 && std::is_move_constructible<Fn>::value >>
	    {
	        ...
	        wrapper(const wrapper& rhs) noexcept : fn(const_cast<Fn&&>(rhs.fn)) { throw ak::logic_error("never called"); } // hack to initialize fn for non-DefaultContructible types
	        wrapper& operator=(const wrapper&) noexcept { throw ak::logic_error("never called"); }
