...
namespace avk
{
	...
	{
		inline input_binding_location_data& from_buffer_at_binding(uint32_t aBindingIndex)
		{
			mGeneralData.mBinding = aBindingIndex;
			return *this;
		}
	 ...
	 */
	inline input_binding_location_data vertex_input_location(uint32_t aLocation, size_t aOffset, vk::Format aFormat, size_t aStride)
	{
		return input_binding_location_data{ 
			{ 0u, aStride, input_binding_general_data::kind::vertex},
			{ aLocation, aOffset, aFormat } 
		};
	}

	// TODO: Figure out how to use this best
	template <class M>
	inline input_binding_location_data vertex_input_location(uint32_t aLocation, const M&)
	{
		return vertex_input_location(aLocation, 0, format_for<M>(), sizeof(M));
	}
	...
	input_binding_location_data vertex_input_location(uint32_t aLocation, M T::* aMember)
	{
	...
	*	The binding point is set to 0 in this case (opposed to `cgb::vertex_input_bindign` where you have to specify it),
	 ...
	 *	The binding point represents a specific buffer which provides the data for the location specified.
	*/
	inline input_binding_location_data instance_input_location(uint32_t aLocation, size_t aOffset, vk::Format aFormat, size_t aStride)
	{
		return input_binding_location_data{ 
			{ 0u, aStride, input_binding_general_data::kind::instance}, 
			{ aLocation, aOffset, aFormat } 
		};
	}
